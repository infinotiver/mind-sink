...
function GalleryGrid({ columns = 4 }: GalleryGridProps) {
  ...
  const {
    ...
    queryFn: () =>
      user?.user_id ? getUserItems(user.user_id) : Promise.reject('User ID is missing'),
  });
  ...
  if (error)
    return (
      <div className="p-4 text-center text-red-700">Failed to load items: {String(error)}</div>
    );
  ...
  return (
    <div
      className={`
      columns-${columns}
      ${columns === 4 ? 'sm:columns-3 lg:columns-4' : `sm:columns-${columns} lg:columns-${columns}`}
      gap-4
      space-y-4
      `}
    >
